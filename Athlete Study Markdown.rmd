...
# Apply your CLR to each sample
ath2_clr <- transform_sample_counts(ath2_rel_abundance_pseudo, clr_transformation)

# (Optional but recommended) drop zero-variance taxa after CLR
otu_mat <- as.matrix(otu_table(ath2_clr))
keep_taxa <- apply(otu_mat, 1, function(v) sd(v) > 0)
ath2_clr <- prune_taxa(keep_taxa, ath2_clr)

# (Optional) keep your Unclassified filter
ath2_clr <- subset_taxa(ath2_clr, Genus != "Unclassified")
...
mean_ci <- function(x, conf = 0.95) {
  ...
  se <- s / sqrt(n)
  z <- qnorm((1 + conf) / 2)
  c(n = n, mean = m, sd = s, lo = m - z * se, hi = m + z * se)
}

# Helper: Cohen's d (Athlete - Non-Athlete)
...
# Function to build one Table 2 row for a variable
make_row <- function(var, label, digits = 1) {
  a <- mean_ci(sample_metadata_df %>% filter(Person_Type == "Athlete") %>% pull(!!sym(var)))
  n <- mean_ci(sample_metadata_df %>% filter(Person_Type == "Non-Athlete") %>% pull(!!sym(var)))

  d <- cohens_d(
    sample_metadata_df %>% filter(Person_Type == "Athlete") %>% pull(!!sym(var)),
    sample_metadata_df %>% filter(Person_Type == "Non-Athlete") %>% pull(!!sym(var))
  )

  # ANOVA p-value (same approach you used originally)
  p <- summary(aov(sample_metadata_df[[var]] ~ sample_metadata_df$Person_Type))[[1]][["Pr(>F)"]][1]

  tibble(
    Measure = label,
    Athlete = sprintf(paste0("%.", digits, "f ± %.", digits, "f (", "%.", digits, "f–%.", digits, "f), n=%d"),
                      a["mean"], a["sd"], a["lo"], a["hi"], a["n"]),
    `Non-Athlete` = sprintf(paste0("%.", digits, "f ± %.", digits, "f (", "%.", digits, "f–%.", digits, "f), n=%d"),
                            n["mean"], n["sd"], n["lo"], n["hi"], n["n"]),
    `Cohen's d` = round(d, 2),
    `P-value` = format(p, scientific = TRUE, digits = 3)
  )
}

# Build the revised Table 2
table2_clean <- bind_rows(
  make_row("Weight_kg", "Weight (kg)", digits = 1),
  make_row("Height_cm", "Height (cm)", digits = 1),
  make_row("Age", "Age (yrs)", digits = 1),
  make_row("BMI", "BMI", digits = 1),
  make_row("VO2max", "VO₂max (mL·kg⁻¹·min⁻¹)", digits = 1)
)

# Display
kable(table2_clean, caption = "Table 2. Comparison of physical and physiological measures between athletes and non-athletes. Values are mean ± SD (95% CI), with effect sizes (Cohen’s d) and ANOVA p-values.")
...
#### Prepare the phyloseq for analysis using a Regression Random Forest by first removing all NAs. To calculate the Mean Squared error (MSE), the range was calculated. The training set was prepared, predictor variable was the ASVs from the Gut microbiome and the response variable was the Fat Free Percent (body composition). Using the MSE the importance scores were extracted from the model. The top ten taxa are filtered out and observed against Fat Free Percent using spearman rank correlations. Model was validated against 1000 permutations of shuffled response variable. 
...
#### Filter samples for Random Forest analysis
# Retain only samples with valid body composition data.
# This step removes any samples where Fat_Free_Percent is missing (NA)
# or falls outside the biologically plausible range (0–100).
# The resulting ath2_BC object contains the CLR-transformed microbial abundances
# and only those samples suitable for regression modeling of Fat_Free_Percent.
ath2_BC <- subset_samples(ath2_clr,
                          !is.na(Fat_Free_Percent) &
                          Fat_Free_Percent >= 0 &
                          Fat_Free_Percent <= 100)
...
# Create a taxa dataframe (from CLR object)
...
# Plot Spearman vs CLR abundance (no log10)
Fig1B <- ggplot(ath.genera3_filtered, aes(x = Fat_Free_Percent, y = Abundance)) +
  ...
  facet_wrap(~ Genus, scales = "free_y",
             labeller = as_labeller(function(x) paste0("italic('", x, "')"),
                                    default = label_parsed)) +
  labs(y = "CLR abundance", x = "Fat Free Percent") +
  theme_bw()

Fig1B

# View MSE contribution 
imp_BC_top10 <- head(imp_BC_merged, 10)
print(imp_BC_top10[, c("Genus", "X.IncMSE")])
```


## Permutation test for Random Forest model validation for Fat Free Mass

### Then, a 1,000-iteration permutation test is performed by repeatedly
### shuffling the response variable (res3) and refitting the Random Forest.
### Each iteration computes a "null" R², representing what would be expected
### under no true association between microbial abundances and fat-free mass.
### The permutation p-value represents the proportion of permuted R² values
### greater than or equal to the observed R², indicating whether the model's
### performance is significantly better than random.

```{r}
set.seed(8)
obs_r2 <- 1 - class.BC$mse[which.min(class.BC$mse)] / var(res3)  # Observed R²
...
for (i in seq_len(n_perm)) {
  perm_res <- sample(res3)  # Shuffle response variable
  rf_perm <- randomForest(perm_res ~ ., data = machine.data3, ntree = 1000)
  perm_r2[i] <- 1 - rf_perm$mse[which.min(rf_perm$mse)] / var(perm_res)
}

# Calculate permutation-based p-value
p_val <- mean(perm_r2 >= obs_r2)

# Print results
cat("Observed R²:", round(obs_r2, 3), "\n")
...
#### Filter samples for Random Forest analysis
# Retain only samples with valid VO2max data (non-missing).
# The resulting ath2_VO2max object contains CLR-transformed microbial abundances
# and only those samples suitable for regression modeling of VO2max.
ath2_VO2max <- subset_samples(ath2_clr, !is.na(VO2max))

# (Optional but recommended) Ensure 'Unclassified' taxa are removed for interpretability.
# If your CLR object already removed these upstream, this will do nothing.
ath2_VO2max <- subset_taxa(ath2_VO2max, Genus != "Unclassified")

# Quick check
head(sample_data(ath2_VO2max))

## Range of VO2max (for context / scale of the response variable)
...
cat("Maximum VO2max:", max_value_V, "\n")
cat("Range of VO2max:", range_value_V, "\n")

# Make Training Dataset (CLR abundances)
...
# Remove 'X' prefix from OTU IDs if present
...
# Figure 1C - IncMSE bar plot
Fig1C <- ggplot(imp_VO2max_top10, aes(x = reorder(Genus, X.IncMSE, decreasing = TRUE),
                                     y = X.IncMSE, fill = Order)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  theme_classic() +
  labs(x = expression(italic("Genus")),
...
# Figure 1D - Spearman correlations for top 10 genera vs VO2max (CLR abundances)
top_genera2 <- unique(imp_VO2max_top10$Genus)
...
Fig1D <- ggplot(ath.genera2_filtered, aes(x = VO2max, y = Abundance)) +
  ...
  facet_wrap(~ Genus, scales = "free_y",
             labeller = as_labeller(function(x) paste0("italic('", x, "')"),
                                    default = label_parsed)) +
  labs(y = "CLR abundance", x = expression(VO[2]*"max (mL·kg⁻¹·min⁻¹)")) +
  theme_bw()

Fig1D

# View IncMSE values
print(imp_VO2max_top10[, c("Genus", "X.IncMSE")])
```

## Permutation test for Random Forest model validation for Vo2Max

### Tests whether the observed model R² could arise by chance.
### Observed R² is calculated from the fitted model.
### A 1,000-iteration permutation test is then performed by shuffling VO2max (res2),
### refitting the Random Forest each time, and computing a null R² distribution.
### The permutation p-value is the proportion of permuted R² values >= observed R².

```{r}
set.seed(8)
obs_r2_vo2 <- 1 - class.VO2max$mse[which.min(class.VO2max$mse)] / var(res2)

n_perm <- 1000
perm_r2_vo2 <- numeric(n_perm)

for (i in seq_len(n_perm)) {
  perm_res <- sample(res2)
  rf_perm <- randomForest(perm_res ~ ., data = machine.data2, ntree = 1000)
  perm_r2_vo2[i] <- 1 - rf_perm$mse[which.min(rf_perm$mse)] / var(perm_res)
}

p_val_vo2 <- mean(perm_r2_vo2 >= obs_r2_vo2)

cat("Observed R² (VO2max):", round(obs_r2_vo2, 3), "\n")
cat("Permutation p-value (VO2max):", p_val_vo2, "\n")
...
Fig1A_with_legend <- ggarrange(Fig1A_clean, legend_A, 
                               ncol = 2, widths = c(3, 2.0))  # Keep legend close
...
Fig3B_clean <- Fig3B_clean + theme(
  ...
  plot.margin = unit(c(10, 10, 50, 10), "pt")  # top, right, bottom, left
)
...
## Permutation test for Random Forest model validation for Person_Type (Athlete v. Non-Athlete)
### Observed model performance is calculated from the fitted Random Forest
### classification model using out-of-bag (OOB) accuracy.
### A 1,000-iteration permutation test is then performed by randomly shuffling
### the class labels (Person_Type), refitting the Random Forest at each iteration,
### and computing a null distribution of classification accuracy.
### The permutation p-value represents the proportion of permuted accuracies
### greater than or equal to the observed accuracy.

```{r}
set.seed(8)

# ensure fixed factor levels
machine.data_top20$res <- factor(machine.data_top20$res,
                                 levels = c("Athlete","Non-Athlete"))

# observed model
rf_obs <- randomForest(res ~ ., data = machine.data_top20, ntree = 1000)
oob_pred_obs <- rf_obs$predicted
cm_obs <- table(True = machine.data_top20$res, Pred = oob_pred_obs)

sens_obs <- cm_obs["Athlete","Athlete"] / sum(cm_obs["Athlete",])
spec_obs <- cm_obs["Non-Athlete","Non-Athlete"] / sum(cm_obs["Non-Athlete",])
bal_acc_obs <- (sens_obs + spec_obs) / 2

# permutation
n_perm <- 1000
perm_balacc <- numeric(n_perm)

for (i in seq_len(n_perm)) {
  perm_res <- sample(machine.data_top20$res)
  perm_res <- factor(perm_res, levels = levels(machine.data_top20$res))

  dat_perm <- machine.data_top20
  dat_perm$res <- perm_res

  rf_perm <- randomForest(res ~ ., data = dat_perm, ntree = 1000)
  oob_pred <- rf_perm$predicted

  cm <- table(True = dat_perm$res, Pred = oob_pred)

  # safety: ensure all cells exist
  for (lv in levels(dat_perm$res)) {
    if (!lv %in% rownames(cm)) cm <- rbind(cm, setNames(rep(0, ncol(cm)), colnames(cm)))
    if (!lv %in% colnames(cm)) cm <- cbind(cm, setNames(rep(0, nrow(cm)), lv))
  }
  cm <- cm[levels(dat_perm$res), levels(dat_perm$res)]

  sens <- cm["Athlete","Athlete"] / sum(cm["Athlete",])
  spec <- cm["Non-Athlete","Non-Athlete"] / sum(cm["Non-Athlete",])
  perm_balacc[i] <- (sens + spec) / 2
}

p_val <- mean(perm_balacc >= bal_acc_obs)

cat("Observed OOB balanced accuracy:", round(bal_acc_obs, 3), "\n")
cat("Permutation p-value:", p_val, "\n")
cat("Max permuted balanced accuracy:", max(perm_balacc), "\n")
summary(perm_balacc)
...
#### Multilayer Perceptron (neural network) with 5-fold CV 
#### - Adds stratified 5-fold cross-validation
####  - Upsampling happens ONLY inside each training fold (prevents leakage)
#### - Scaling (center/scale) fit on training fold, applied to validation fold
####  - Keeps a final 80/20 split to generate Fig 4A–C (confusion matrix, ROC, heatmap)

```{r}

# -------------------------------------------------------------------
# Reproducibility / determinism (set BEFORE loading tensorflow/keras)
# -------------------------------------------------------------------
Sys.setenv(
...
# ------------------------------------------------------------
# 1) Build CLR-transformed feature matrix (samples x taxa)
#    IMPORTANT: Do NOT re-normalize CLR to relative abundance.
# ------------------------------------------------------------
...
X <- as.matrix(otu_table(ath2_top20))
if (taxa_are_rows(ath2_top20)) X <- t(X)  # ensure rows = samples, cols = taxa

# Response labels (factor with consistent ordering)
y <- factor(sample_data(ath2_top20)$Person_Type, levels = c("Athlete", "Non-Athlete"))
...
# ------------------------------------------------------------
# Helper: build a fresh model (called each fold)
# ------------------------------------------------------------
build_mlp <- function(input_shape) {
...
# ------------------------------------------------------------
# Helper: standardize based on training fold only
# ------------------------------------------------------------
...
apply_scaler <- function(mat, scaler) {
  sweep(sweep(mat, 2, scaler$mu, "-"), 2, scaler$sdv, "/")
}

# ------------------------------------------------------------
# 2) 5-fold stratified cross-validation (no leakage)
# ------------------------------------------------------------
set.seed(123)
folds <- createFolds(y, k = 5, list = TRUE, returnTrain = FALSE)

cv_results <- vector("list", length(folds))
...
for (k in seq_along(folds)) {
  ...
  # Upsample TRAINING ONLY (prevents leakage) 
  train_df <- as.data.frame(X_train)
  up <- upSample(x = train_df, y = y_train)  # returns predictors + "Class"
  y_train_up <- factor(up$Class, levels = levels(y))
  X_train_up <- as.matrix(up[, setdiff(colnames(up), "Class"), drop = FALSE])

  # Scale using TRAINING fold only 
  scaler <- fit_scaler(X_train_up)
  X_train_up_sc <- apply_scaler(X_train_up, scaler)
  X_val_sc      <- apply_scaler(X_val, scaler)

  # Encode labels: Athlete = 0, Non-Athlete = 1 
  y_train_num <- ifelse(y_train_up == "Athlete", 0, 1)
  y_val_num   <- ifelse(y_val == "Athlete", 0, 1)

  # Fit model 
  tensorflow::set_random_seed(1234 + k)  # fold-specific but reproducible
  model <- build_mlp(input_shape = ncol(X_train_up_sc))
...
history <- model %>% fit(
  ...
  val_prob <- as.numeric(model %>% predict(X_val_sc))
  val_pred <- ifelse(val_prob > 0.5, 1, 0)

  # Metrics 
  cm <- confusionMatrix(
    data = factor(val_pred, levels = c(0, 1)),
    reference = factor(y_val_num, levels = c(0, 1)),
    positive = "1"
  )

  bal_acc <- as.numeric(cm$byClass["Balanced Accuracy"])

  roc_obj <- roc(response = y_val_num, predictor = val_prob, quiet = TRUE)
...
cat("Balanced accuracy:", round(mean(cv_summary$bal_acc), 3), "±", round(sd(cv_summary$bal_acc), 3), "\n")
cat("AUC:",              round(mean(cv_summary$auc), 3),     "±", round(sd(cv_summary$auc), 3), "\n")

# ------------------------------------------------------------
# 3) Final 80/20 split for Figure 4A–C (no leakage)
# ------------------------------------------------------------
set.seed(123)
train_index <- createDataPartition(y, p = 0.8, list = FALSE)

X_train0 <- X[train_index, , drop = FALSE]
y_train0 <- y[train_index]
X_test0  <- X[-train_index, , drop = FALSE]
y_test0  <- y[-train_index]

# Upsample training only
train_df0 <- as.data.frame(X_train0)
up0 <- upSample(x = train_df0, y = y_train0)
y_train_up0 <- factor(up0$Class, levels = levels(y))
X_train_up0 <- as.matrix(up0[, setdiff(colnames(up0), "Class"), drop = FALSE])

# Scale using training only
scaler0 <- fit_scaler(X_train_up0)
X_train_up0_sc <- apply_scaler(X_train_up0, scaler0)
X_test0_sc     <- apply_scaler(X_test0, scaler0)

# Encode labels
y_train_num0 <- ifelse(y_train_up0 == "Athlete", 0, 1)
y_test_num0  <- ifelse(y_test0 == "Athlete", 0, 1)

# Train final model
tensorflow::set_random_seed(1234)
final_model <- build_mlp(input_shape = ncol(X_train_up0_sc))
...
# Predict test set
test_prob <- as.numeric(final_model %>% predict(X_test0_sc))
test_pred <- ifelse(test_prob > 0.5, 1, 0)

# Confusion matrix (numeric)
conf_matrix <- confusionMatrix(
  data = factor(test_pred, levels = c(0, 1)),
  reference = factor(y_test_num0, levels = c(0, 1)),
  positive = "1"
)
print(conf_matrix)

# ----------------------
# Fig 4A: Confusion plot
# ----------------------
cm_table <- as.data.frame(conf_matrix$table)
colnames(cm_table) <- c("Prediction","Reference","Freq")

cm_table$Prediction <- factor(cm_table$Prediction, levels = c("0","1"), labels = c("Athlete","Non-Athlete"))
cm_table$Reference  <- factor(cm_table$Reference,  levels = c("0","1"), labels = c("Athlete","Non-Athlete"))

fig4A <- ggplot(cm_table, aes(x = Reference, y = Prediction, fill = Freq)) +
...
# ----------------------
# Fig 4B: ROC / AUC (ggplot object)
# ----------------------
roc_obj <- roc(response = y_test_num0, predictor = test_prob, quiet = TRUE)
...
fig4B <- ggplot(roc_df, aes(x = fpr, y = tpr)) +
...
# ----------------------
# Fig 4C: Heatmap (top 50 by |FC|)
#   - Uses CLR for fold-change ranking (difference in mean CLR)
#   - Computes % contribution more stably by averaging exp(CLR) FIRST
#     (avoids exp(mean(CLR)) artifacts)
# ----------------------

# Put test set back into a data frame (use unscaled CLR, not scaled z-scores)
df_test <- as.data.frame(X_test0)  # CLR values for test samples
df_test$Predicted <- factor(test_pred, levels = c(0,1), labels = c("Pred_Athlete","Pred_NonAthlete"))
df_test$SampleID <- rownames(df_test)

# Map feature IDs -> Genus
taxa_df <- as.data.frame(tax_table(ath2_top20))
otu_to_genus <- taxa_df$Genus
names(otu_to_genus) <- rownames(taxa_df)
otu_to_genus[is.na(otu_to_genus)] <- "Unknown"

# Long format (CLR abundances)
df_long <- df_test %>%
  pivot_longer(
    cols = -c(SampleID, Predicted),
    names_to = "OTU",
    values_to = "Abundance"
  ) %>%
...
# Compute fold-change in CLR space (difference in mean CLR) 
df_summary_clr <- df_long %>%
  group_by(Predicted, Genus) %>%
  summarize(mean_clr = mean(Abundance, na.rm = TRUE), .groups = "drop")

df_wide_clr <- df_summary_clr %>%
  pivot_wider(
    id_cols = Genus,
  ...
  mutate(FC = Pred_NonAthlete - Pred_Athlete)

# Compute % contribution by averaging in exp(CLR) space first 
df_long_exp <- df_long %>%
  mutate(exp_abund = exp(Abundance))

df_summary_exp <- df_long_exp %>%
  group_by(Predicted, Genus) %>%
  summarize(mean_exp = mean(exp_abund, na.rm = TRUE), .groups = "drop")

df_wide_exp <- df_summary_exp %>%
  pivot_wider(
    id_cols = Genus,
    names_from = Predicted,
...
# Top 50 genera based on |FC| (CLR ranking)
topN <- 50
top_genera <- df_wide_clr %>%
  arrange(desc(abs(FC))) %>%
  slice(1:topN) %>%
  pull(Genus)

# Build matrices for heatmap
df_exp_top <- df_wide_exp %>% filter(Genus %in% top_genera)
df_fc_top  <- df_wide_clr %>% filter(Genus %in% top_genera)

# Keep a consistent ordering: sort by |FC|
df_fc_top <- df_fc_top %>% arrange(desc(abs(FC)))
df_exp_top <- df_exp_top %>%
  mutate(Genus = factor(Genus, levels = df_fc_top$Genus)) %>%
  arrange(Genus)

# Matrix of mean exp(CLR) values
mat_exp <- as.matrix(df_exp_top[, c("Pred_Athlete", "Pred_NonAthlete")])
rownames(mat_exp) <- as.character(df_exp_top$Genus)

# Convert to % contribution within each predicted class
mat_percent <- sweep(mat_exp, 2, colSums(mat_exp), FUN = "/") * 100

# Fold-change values (CLR)
fc_values <- df_fc_top$FC
names(fc_values) <- df_fc_top$Genus

# Heatmap colors
col_fun <- colorRamp2(
  c(min(mat_percent), median(mat_percent), max(mat_percent)),
  c("blue", "white", "red")
)

row_anno <- rowAnnotation(
  Fold_Change = anno_barplot(fc_values, border = FALSE, bar_width = 0.8)
)

fig4C <- Heatmap(
  mat_percent[as.character(df_fc_top$Genus), , drop = FALSE],
  name = "% Contribution",
  col = col_fun,
