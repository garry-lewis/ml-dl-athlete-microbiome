...
# Perform ANOVA tests on Weight, Height, Age, BMI, and VO2max between athletes and non-athletes
...
anova_bmi <- aov(BMI ~ Person_Type, data = sample_metadata_df)
anova_vo2 <- aov(VO2max ~ Person_Type, data = sample_metadata_df)
...
p_value_vo2 <- summary(anova_vo2)[[1]][["Pr(>F)"]][1]
...
significance_bmi <- ifelse(p_value_bmi < 0.05, "Significant", "Not Significant")
significance_vo2 <- ifelse(p_value_vo2 < 0.05, "Significant", "Not Significant")

# Create a summary table for ANOVA results
anova_results_table <- data.frame(
  Measure = c("Weight", "Height", "Age", "BMI", "VO2max"),
  P_Value = c(p_value_weight, p_value_height, p_value_age, p_value_bmi, p_value_vo2),
  Significance = c(significance_weight, significance_height, significance_age, significance_bmi, significance_vo2)
)
